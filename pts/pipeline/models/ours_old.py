...
class OurDualModel:
    def __init__(self, diff_id: str, llm_id: str, max_new_latents: int = 128, 
        ...
        if "Dream" in diff_id or "dream" in diff_id:
            # Dream model is a diffusion LM, uses custom DreamModelLM wrapper
            ...
            # LLaDA model uses custom LLaDAModelLM
        ...
        # Load base LLM model, allowing unused weights from latents_projector
        ...
        # Latents projector: only use if the model provides a trained projector
        if hasattr(self.llm_model, "latents_projector"):
            print("[INFO] Loading latents_projector from model")
            self.latents_projector = self.llm_model.latents_projector.to("cuda").to(torch.bfloat16)
        else:
            # Try to reconstruct and load the latents_projector from checkpoint
            self._load_or_create_latents_projector()
    ...
    def generate_answer(self, user_prompt: str, latents=None, system_prompt=None, use_latents=True, **kwargs) -> Dict:
        ...
        if use_latents and latents is not None:
            ...
            # Concatenate latents before text embeddings
